...
#ifndef __ANYTIME_DUAL_QUEUE_SINGLE_HEURISTIC_BEST_FIRST_SEARCH_TRACER__
#define __ANYTIME_DUAL_QUEUE_SINGLE_HEURISTIC_BEST_FIRST_SEARCH_TRACER__
...
public:
	...
	AT_BFS_DQ_SH_TRACER( 	const Search_Model& search_problem )
	...
	AT_BFS_DQ_SH_TRACER( 	const Search_Model& search_problem, Abstract_Heuristic& h )
	: m_problem( search_problem ), m_heuristic_func(&h),
	m_exp_count(0), m_gen_count(0), m_pruned_B_count(0), m_dead_end_count(0), m_open_repl_count(0),
	m_B( infty ), m_time_budget(infty), m_po_exp_left( 100 ), m_non_po_exp_left(0), m_po_exp_max(100), m_non_po_exp_max(0) {
	}

	virtual ~AT_BFS_DQ_SH_TRACER() {
	...
	bool	find_solution( float& cost, std::vector<Action_Idx>& plan, std::vector<State>& trace ) {
		cost = infty;
		m_t0 = time_used();
		Search_Node* end = do_search();
		if ( end == NULL ) return false;
		extract_plan( m_root, end, plan, cost );
    extract_trace( m_root, end, trace);
		float t2 = time_used();
...
protected:
  ...
  void	extract_trace( Search_Node* s, Search_Node* t, std::vector<State>& trace ) {
    Search_Node *tmp = t;
    std::vector<State> rev_trace;
    while( tmp != s) {
      rev_trace.push_back( * (tmp->state()));
      tmp = tmp->parent();
    }

    for(auto it = rev_trace.rbegin(); it != rev_trace.rend(); ++it){
      trace.push_back(*it);
    }
